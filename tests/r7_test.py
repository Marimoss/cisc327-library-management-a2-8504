...
def test_get_patron_status_report_one():
    ...
    add_book_to_catalog("Detective Chinatown", "Peak Director", "8888888888888", 1)  # Add new books to borrow.
...
def test_get_patron_status_report_returned():
    ...
    return_book_by_patron("888888", 4)  # Return the previously borrowed book above.
    report = get_patron_status_report("888888")
...
def test_get_patron_status_report_many():
    ...
    borrow_book_by_patron("888888", 5)
    borrow_book_by_patron("888888", 6)
    return_book_by_patron("888888", 5)  # Return two books on time, already one book in history from previous test. 
    return_book_by_patron("888888", 6)

    # Simulate a late return scenario.
    borrowed = datetime.now() - timedelta(days=21)
    due_date = borrowed + timedelta(days=14) 
    insert_borrow_record("888888", 7, borrowed, due_date)  # Already due 7 days ago.
    ...
    assert report['current_borrowed_count'] == 1
    assert len(report['borrow_history']) == 3 
